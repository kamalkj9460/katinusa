...
$(() => {
  ...
  if (window.PXUTheme.theme_settings.search_pagination_type === 'load_more_button') {
    window.enableLoadMoreButton('.search-matrix');
  }

  if (window.PXUTheme.theme_settings.search_pagination_type === 'load_more') {
    window.enableLoadMoreSearch();
  }

  if (window.PXUTheme.theme_settings.search_pagination_type === 'infinite_scroll') {
  ...
  function refreshCart(cart) {
    ...
    window.updateFreeShippingProgressIndicator()
  }
...
$(document)
  .on('shopify:block:select', function(e){
    ...
    if($parentSection.hasClass('shopify-section--page-details-template')) {
      ...
      if($('.recently-viewed__section').length && window.PXUTheme.theme_settings.collection_secondary_image) {
        window.imageFunctions.showSecondaryImage();
      }
    }
...
$(() => {
  window.updateFreeShippingProgressIndicator = () => {
    const freeShippingPrice = 75 * 100;

    const showIndicator = (percentage, dollarsRemaining) => {  
      const freeShippingText = percentage >= 100 ? "Congrats! You've got free shipping." : "You're $" + dollarsRemaining + " away from getting free shipping."
      $(".FreeShippingProgressIndicatorText").text(freeShippingText)

      setTimeout(() => { 
        $(".FreeShippingProgressIndicatorBar").css("width", percentage + "%")
      }, 0);
    }

    $.getJSON('/cart.js', function(cart) {
      const has_free_shipping = cart.total_price >= freeShippingPrice;
      if (has_free_shipping) { 
        showIndicator(100);
      } else { 
        const percentage = parseInt(cart.total_price/freeShippingPrice * 100)
        const dollarsRemaining = (freeShippingPrice - cart.total_price) / 100
        showIndicator(percentage, dollarsRemaining)
      }
    });
  }
...
//     ]
//     .map(s => s.toUpperCase())
//     .map(s => s.replace(" ", encodeURIComponent("+")))

const swatch_param_entry_key = 'filter.v.option.color'
const color_swatch_groupings_text = `black: zebra marl, black wash, black, black marl, black marle, black wash mineral, black white, charcoal, zebra marl, zebra marle
blue: storm, baltic blue, holland blue, blue cloud, blue surf, overcast, overcast blue, steel blue wool, light blue, sky blue mineral, indigo wash, steel blue, baltic blue cloud, steel blue mineral, soot, soot cloud, tradewinds, indigo, blue surf, dark teal, indigo wash, lt blue cloud, steel blue, teal, foam, slate blue, marina, new navy, steel blue/warm gray, steel blue/wool, blue/red, light blue
brown: coffee, bark, dark brown marl, walnut, mocha, brown, umber, dark red, dark red marl, umber mineral, walnut, dark brown, gravel/dark clay
gray: cement cloud, cement, glass gray, warm gray, graphite, gravel, cedar, light gray, light grey, light gray, charcoal, heather gray, gray cloud, gray green, soot, glass gray, slate, soot cloud, dark heather gray, cement heather
green: light green, olive, pine, army, jade, cactus, gray green, olive mineral, pimento, eden, kelly green, teal
lavender: lavender, lavender mineral
multi: speckle
navy: slate, baltic blue, navy, new navy, baltic blue cloud, navy cloud, navy marl, navy mineral, navy marle
orange: burnt orange, mineral red
pink: pink cement mineral, pink, cherry blossom mineral, lt peach, dusty pink, lt peach cloud, pigment pink, rose smoke, mauve, rose smoke cloud, pink mineral, dusty pink, pink cement cloud, pink cement mineral
purple: purple haze, lavender, lavender mineral, dusk
red: red, dark clay, cherry blossom mineral, dark clay wool, dark red marl, dark red marle, dark red, mauve, mineral red, putty, rose smoke cloud, rose smoke wool, rose, kelp red, rust, kelp red mineral, sedona red
tan: almond, tan cloud, khaki mineral, putty, khaki , camel, brass marl, driftwood, tan cloud, wool cloud
tie-dye: baltic blue tie dye, multi tie dye, rose smoke tie dye, smog tie dye, steel blue tie dye, tan tie dye, teal blue tie dye, teal tie dye, tie dye
white: white, wool, wool black, wool dark clay, wool lavender, wool rose smoke, wool/blackwash, vintage white, wool cloud
yellow: brass cloud, lager mineral, yellow, driftwood cloud, brass, honey, mustard, lime, brass marl, driftwood, khaki mineral, khaki, butter, gold/green`
const color_lines = color_swatch_groupings_text
.split("\n")
.map(line => { 
  return { 
    label: line.split(":")[0].trim(),
    colors: line.split(":")[1]?.split(",").map(s => s.toUpperCase())
    .map(s => s.replace(" ", "+"))
  }
})

const convert_comma_string_to_array = (swatch_list_raw) => { 
  const list = swatch_list_raw
    .split(",")
...
window.onClickedCustomFilterCheckbox = (e) => { 
  e.preventDefault();
  const target = e.target;

  if (target.checked) { 
    add_filter_to_url(target.name, target.value)
  } else { 
    remove_filter_from_url(target.name, target.value)
  }

  return;
}

const add_filter_to_url = (name, values) => { 
  const array = convert_comma_string_to_array(values)
...
const remove_filter_from_url = (name, values) => { 
  const array = convert_comma_string_to_array(values)
  
  const url = new URL(window.location.href);
  var values_updated = url.searchParams.getAll(name).map(v => v.replace(' ', '+'))
  
  for (const removing_value of array) { 
    values_updated.splice(values_updated.indexOf(removing_value), 1);
  }
  url.searchParams.delete(name);
  values_updated.forEach(value => url.searchParams.append(name, value));
  
  window.location.href = decodeURIComponent(url.href)
}

const update_checked_statuses_of_color_swatches = () => {
  const url = new URL(window.location.href);
  const all_colors = url.searchParams.getAll(swatch_param_entry_key)

  for (const line of color_lines) { 
    const element = $(`#swatch-filter-${line.label}`)
    const first_specific_swatch = line.colors[0].replace("+", ' ').trim()
    
    const should_be_checked = all_colors.includes(first_specific_swatch)
    // console.log(all_colors, first_specific_swatch, should_be_checked)
    element.attr("checked", should_be_checked);
  }
}

$(() => { 
  update_checked_statuses_of_color_swatches()
})

const debugURLSearchParams = (params) => { 
  for(var pair of params.entries()) {
    console.log(pair[0]+ ', '+ pair[1]);
 }
}

window.onClickedCustomCollectionSort = (e) => { 
  e.preventDefault();
  const target = e.target;
...
//-- get the current variant_id and display the desc line 
function abc_display_var_desc(){
  var curr_variant_id = $('select[name="id"]').val();
  $('.variant_description_line').hide();
  $('.variant_description_line[data-variant-id="'+ curr_variant_id +'"]').show();  
}

//-- on changing the variant
$('select[name="id"]').on('change', function(){

  abc_display_var_desc();
...
var TIMER_abc_avail_check; 

//-- check availability.. executed after a short duration
function abc_check_availability(){
    clearTimeout( TIMER_abc_avail_check );

    setTimeout( abc_DO_check_availability, 100 );
}

//-- check availability and display the NOTIFY button based on it
function abc_DO_check_availability(){

    const queryString = window.location.search;
    const urlParams = new URLSearchParams(queryString)
    const selected = urlParams.get('variant');

    // var selected_variant = $('select[name="id"]').val();
    var selected_variant = selected;

    //-- fix
    selected_variant = null;
  
    //-- when "variant" not available in URL
    if( selected_variant === null ){
      var selected_variant_ele = $('.swatch_options .swatch:nth-child(1) input[type="radio"]:checked+.swatch-element');
      selected_variant = $(selected_variant_ele).data('id');
      
      console.log('~~~sv', selected_variant);
      if( ! $(selected_variant_ele).hasClass('soldout') ){
        console.log('~~~nope');
        selected_variant = $('.swatch_options .swatch:nth-child(2) input[type="radio"]:checked+.swatch-element').data('id');
      }
      else{
        console.log('~~~yes');
      }
    ...
    var is_soldout = $('.swatch-element[data-id="'+ selected_variant +'"]').hasClass('soldout');

    console.log('~~~v = ' + selected_variant + ', sold? = ' + is_soldout);

    $('#abc_notify_btn').data('variant_id', selected_variant);
...
//-------------------------------------------------------------------------------
//-- Free Gift Addon - Cart
// function abc_gift_prod_cart(){
...
//-- on page load, add the button
$(function(){

    //-- on clicking the submit button
    $('.purchase-details').on('click', '#btn_abc_notify_submit', function(e){
        e.preventDefault();
        $('#abc_modal_notify').hide();

        var cust_email = $.trim( $('#in_cust_email').val() );
        var selected_variant = $('#abc_notify_btn').data('variant_id');
        ...
        */
        ...
        .then(response => {
            ...
            alert('Thank you! We will send you a notification when the product is available!')
        })
        .catch(err => {
            console.error(err);
        });
    ...
    //-- on clicking the Notify Btn to open up the modal
    $('.purchase-details').on('click', '#abc_notify_btn', function(e){
        e.preventDefault();
        $('#abc_modal_notify').css('display', 'flex');
    });

    //-- on clicking the close btn
    $('.purchase-details').on('click', '#btn_close_notify_modal', function(e){
        e.preventDefault();
        $('#abc_modal_notify').fadeOut();
    });

    if( $('.purchase-details__buttons').length > 0 ){    
        // $('.purchase-details__buttons').append(`
        $('.purchase-details').append(`
            <a id="abc_notify_btn" class="global-button global-button--primary" style="display: none; padding: 10px 5px; width: 100%;">Notify Me When Available</a>
            <div id="abc_modal_notify">
                <div class="abc_modal_notify_content">
                    <div id="btn_close_notify_modal">x</div>
                    <p>Register to receive a notification when this item comes back in stock.</p>
                    <input id="in_cust_email" type="email" name="email" placeholder="Email" />
                    <a href="#" id="btn_abc_notify_submit" class="global-button global-button--primary">Notify</a>
                </div>
            </div>
    ...
    //-- on pressing the size buttons, check the availability
    $('.product_form .swatch_options .swatch-element').on('click', function(){
        // console.log('clicked....');
        abc_check_availability();
    });
